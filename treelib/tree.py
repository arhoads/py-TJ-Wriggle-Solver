...
class Tree(object):
    ...
    def create_node(self, tag=None, identifier=None, parent=None, action=None, state=None, pathCost=0):
        """
        Create a child node for the node indicated by the 'parent' parameter.
        """
        node = Node(tag, identifier, action=action, state=state, pathCost=pathCost)
